  ...
  async submit() {
    ...
    if (this.form.valid) {
      ...
      try {
        ...
        // Generar un ID único si el cliente no tiene uno
        if (!clienteData.id || clienteData.id.trim() === '') {
          clienteData.id = this.firebaseSvc.generateId(); // Generar ID único
        }
